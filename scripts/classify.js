...
if (!fs.existsSync(SRC)) {
  ...
  console.error(
    "    Download the latest Excel from Google Sheets and place it in the project root.",
  );
  process.exit(1);
}
...
function classifyCategory(name, desc) {
  ...
  // 2. Keyword matching
  if (/horror|granny|haunted house|ghost|survival horror|no escape.*forest/.test(text))
    return "Horror";
  if (
    /island beast runner|zombie.*apoc|open world.*survival|survival game|go back.*servival/.test(
      text,
    )
  )
    return "Survival";
  if (
    /racing game|race against|race.*ai|race.*track|street.*rebuild.*racing|road rush|racing car/.test(
      text,
    )
  )
    return "Racing";
  if (
    /endless.?run|runner game|\barcade\b|catch.*star|falling.*star|dotix|penguin.*run|bullet.*fracture|hop.*game|mouse.*runner|cube.*runner|neon.*rush|endless.*driving/.test(
      text,
    )
  )
    return "Arcade";
  if (
    /\bpuzzle\b|wire.*cut|bomb.*defus|maze|labyrinth|slide.*solve|arrange.*block|one wire cut/.test(
      text,
    )
  )
    return "Puzzle";
  if (/platformer|2d.*platform|fruit.*survival|5 levels.*story|jump.*enemies/.test(text))
    return "Platformer";
  if (/\brpg\b|role.?play|stealth.*rpg|agent.*247/.test(text)) return "RPG";
  if (/simulation|simulator|parking.*game|developer.*life|smartpark/.test(text))
    return "Simulation";
  if (/\bstrategy\b|board game|ludo|chess/.test(text)) return "Strategy";
  if (
    /\bfps\b|first.?person.*shoot|shoot.*game|\baction\b|combat|parkour|stealth.*action|3d.*action|action.*adventure/.test(
      text,
    )
  )
    return "Action";

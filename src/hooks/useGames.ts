...
export function useGames() {
  // Start with fallback data so games show instantly even before fetch completes
  ...
  useEffect(() => {
    async function fetchGames() {
      try {
        ...
        const CSV_URL = 'https://docs.google.com/spreadsheets/d/19CVf_oS3dByqMt--c8tjNGhGLYcslc-fEpYQe6lEwZg/export?format=csv';
        ...
        rawData.forEach((row: any, index: number) => {
          ...
          const VALID_CATEGORIES = ['Action', 'Arcade', 'Puzzle', 'Horror', 'Racing', 'Platformer', 'RPG', 'Simulation', 'Strategy', 'Survival', 'Adventure'];
          ...
          if (!category) {
            ...
            if (/horror|granny|haunted|ghost|monster.*stalk|survival horror|no escape.*forest/.test(text)) category = 'Horror';
            else if (/racing game|race against|race.*ai|street.*racing|road rush/.test(text)) category = 'Racing';
            else if (/endless.?run|runner game|\barcade\b|falling.*star|catch.*star|dotix|penguin.*run/.test(text)) category = 'Arcade';
            else if (/\bpuzzle\b|wire.*cut|maze|labyrinth|slide.*solve|arrange.*block/.test(text)) category = 'Puzzle';
            ...
            else if (/simulation|simulator|parking.*game|developer.*life/.test(text)) category = 'Simulation';
        ...
        console.warn("Could not load live data from Google Sheets, showing cached games:", err);
        // Keep showing FALLBACK_GAMES (already in state) — don't show error to user
        setLoading(false);
      }
    }
